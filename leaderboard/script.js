...
async function leaderboard(){
    ...
    for(let i=0;i<data.length;i++){
        ...
        if(data[i]['emoji'] != "" && data[i]['emoji'] != null){
            listElement.innerHTML = `<td>${i + 1}</td><td><img src="${"https://openmoji.org/data/color/svg/" + data[i]['emoji'] + ".svg"}" height="30" class="leftImg">${data[i]['username']}<img src="${"https://openmoji.org/data/color/svg/" + data[i]['emoji'] + ".svg"}" height="30" class="rightImg"></td><td>${data[i]['correct']}</td><td>${data[i]['incorrect']}</td>`
        } else {
